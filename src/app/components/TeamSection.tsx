  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="p-4">
          {/* Rol */}
          <div className="text-[9px] uppercase tracking-[0.2em] mb-1" style={{ color: "#38bdf8", fontFamily: "'Orbitron', monospace" }}>
            {member.rol}
          </div>

          {/* Name */}
          <h3 className="text-white font-bold leading-tight mb-1.5" style={{ fontSize: "1.1rem", fontFamily: "'Space Grotesk', sans-serif" }}>
            {member.shortName}
          </h3>

          {/* Description */}
          <p className="text-slate-500 text-[11px] leading-relaxed line-clamp-2" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
            {member.perfil}
          </p>

          {/* Divider */}
          <div className="h-px my-3" style={{ background: `linear-gradient(90deg, ${member.color}30, transparent)` }} />
          ...
          {/* Divider */}
          <div className="h-px my-3" style={{ background: `linear-gradient(90deg, transparent, ${member.color}30)` }} />

          {/* Stats row — only first two */}
          <div className="grid grid-cols-2 gap-2">
            {member.stats.slice(0, 2).map((stat) => (
              <div key={stat.label} className="text-center">
                <div className="text-sm font-bold text-white" style={{ fontFamily: "'Orbitron', monospace" }}>{stat.value}</div>
                <div className="text-[8px] uppercase tracking-[0.12em]" style={{ color: "rgba(255,255,255,0.3)", fontFamily: "'Orbitron', monospace" }}>{stat.label}</div>
              </div>
            ))}
          </div>
          ...
          <motion.div
            className="mt-3 text-[9px] uppercase tracking-[0.15em] text-center opacity-0 group-hover:opacity-100 transition-opacity duration-300"
